...
let rec from_int x = [] (* if x < 0 then clc x else match x with
    0 -> [0]
  | 1 -> [1]
  | _ when x mod 2 = 0 -> 0::from_int (x/2)
  | _ -> 1::from_int (x/2);;

let hihi x = from_int (-x);;

let clc x = let rec oui x = match x with
    [] -> []
  | e::s when e = 1 -> (invert s)
  | e::s -> e::oui s
            in oui (hihi x);;*)
