...
let rec gcd_b bA bB = if  bA=[] || bB= [] then invalid_arg "gcd:inputs must not be zero"
    ...
    let bA= abs_b bA and bB = abs_b bB
    in let c= mod_b bA bB
       in if c=[] then bB else gcd_b bB c else
    let bB= abs_b bA and bA = abs_b bB
    in let c= mod_b bA bB
...
let rec bezout_b bA bB =
  ...
  let g = gcd_b bA bB in
  let d = quot_b bA bB in
  if compare_b r [0;0] = 0 then ([0;0],[0;1],g)
