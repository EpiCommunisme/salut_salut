...
let from_int x =
  let rec from x = match x with
  | 1 -> [1]
  | _ when x mod 2 = 1 -> 1::from (x/2)
  | _ -> 0::from (x/2)
in if x < 0 then 1::from (-x) else 0::from x;;
...
let to_int bA = 0
  (*let rec int x n = match x with
      [] -> 0
    | e::s -> (e*(power 2 n)) + (int s (n+1))
  in (match bA with
        e::s when e = 0 -> int s 0
      | e::s -> -int s 0
      | _ -> 1);;*)
...
let invert bA =
  let rec pri x = function
      [] -> x
    | e::s -> pri (e::x) s
  in pri [] bA;;

let add bA =
  let rec ui x l = match x with
      [] -> []
    | e::s when e = 1 -> ui s (1::l)
    | e::s -> (invert s) @ 1::l
  in ui (invert bA) [];;

let change bA =
  let rec io x l = match x with
      [] -> l
    | e::s when e = 1 -> io s (0::l)
    | e::s -> io s (1::l)
            in io (invert bA) [];;

let print_b bA = ()(*
  let rec pri x p f = if p = 0 then invert x else add (change (invert x))
  in (match bA with
       e::s when e = 0 -> pri s 0 []
     | e::s -> pri s 1 []
     | _ -> []);;*)
